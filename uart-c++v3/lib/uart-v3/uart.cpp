...
size_t Uart::print(long num, int base) {
	if (base == 0) {
		...
		if (num < 0) {
			...
			num = -num;
			return printNumberAschar(num, 10) + t;
		}
		return printNumberAschar(num, 10);
	} else {
		return printNumberAschar(num, base);
	}
}

size_t Uart::print(unsigned long num, int base) {
	if (base == 0) return write(num);
	else return printNumberAschar(num, base);
}
...
size_t Uart::printNumberAschar(unsigned long n, int base) {
...
size_t Uart::writeBytes(float num) {
	writeNum((float)num, 2);
}
