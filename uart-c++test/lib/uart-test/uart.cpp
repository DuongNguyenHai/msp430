...
size_t Uart::print(const char *s, size_t size) {
	return write((const char *)s, (size==-1)? strlen(s): size );
}

size_t Uart::printNumFloat(double num, int digits) {
...
size_t Uart::println(const char *s, size_t size) {
   size_t n = print(s, (size==-1)?strlen(s):size);
	n +=  println();
	return n;
}

size_t Uart::printlnNumFloat(double num, int digits) {
	size_t n = printNumFloat(num, digits);
	n +=  println();
	return n;
}
...
size_t Uart::writeNumber(double num, unsigned char type) {
	_data dt = {0, num};
	size_t n = 0;
...
size_t Uart::writeBytes(unsigned char num) {
	return writeNumber((double)num, 1);
}

size_t Uart::writeBytes(int num) {
	return writeNumber((double)num, 2);
}
//
size_t Uart::writeBytes(unsigned int num) {
	return writeNumber((double)num, 2);
}

size_t Uart::writeBytes(long num) {
	return writeNumber((double)num, 4);
}

size_t Uart::writeBytes(unsigned long num) {
	return writeNumber((double)num, 4);
}

size_t Uart::writeBytes(double num) {
	return writeNumber((double)num, 8);
}

size_t Uart::readNumber(_data &dt, int type) {
...
size_t Uart::readBytes(unsigned char &num) {
	_data dt = {0, num};
	if(size_t n = readNumber(dt, UN_CHAR)) {
